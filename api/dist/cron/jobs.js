...
async function recalcReputation() {
    ...
    for (const user of users) {
        const [reviewCount, positiveCount, negativeCount, completedSales, completedBuys, unpaidCount, disputeCount] = await Promise.all([
            db_1.prisma.review.count({ where: { targetId: user.id } }),
            db_1.prisma.review.count({ where: { targetId: user.id, rating: { gte: 4 } } }),
            db_1.prisma.review.count({ where: { targetId: user.id, rating: { lte: 2 } } }),
            db_1.prisma.deal.count({ where: { sellerId: user.id, status: "COMPLETED" } }),
            db_1.prisma.deal.count({ where: { buyerId: user.id, status: "COMPLETED" } }),
            db_1.prisma.deal.count({ where: { buyerId: user.id, status: "UNPAID_RELISTED" } }),
            db_1.prisma.deal.count({ where: { status: "DISPUTED", OR: [{ buyerId: user.id }, { sellerId: user.id }] } }),
        ]);
