...
const UploadQuery = zod_1.z.object({
...
async function mediaRoutes(app) {
    app.post("/media/upload", { preHandler: guards_1.requireAuth, config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (request, reply) => {
        ...
        const baseDir = path_1.default.join(process.cwd(), UPLOAD_DIR);
        ...
        for (const v of VARIANTS) {
            ...
            variants.push({
                variant: v.type,
                key: path_1.default.join(UPLOAD_DIR, asset.id, outputName),
                mime: "image/webp",
                size: out.length,
        ...
        await db_1.prisma.$transaction(async (tx) => {
            ...
            await tx.mediaAsset.update({
                ...
                data: {
                    status: "READY",
                    storageKey: variants[variants.length - 1]?.key ?? "",
